...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------#
    train_topview_path = '2007_train_tv.txt'
    train_sideview_path = '2007_train_sv.txt'
    val_topview_path = '2007_val_tv.txt'
    val_sideview_path = '2007_val_sv.txt'
    ...
    #   读取数据集对应的txt
    # ---------------------------#
    with open(train_topview_path, encoding='utf-8') as f:
        train_tv_lines = f.readlines()
    with open(train_sideview_path, encoding='utf-8') as f:
        train_sv_lines = f.readlines()
    with open(val_topview_path, encoding='utf-8') as f:
        val_tv_lines = f.readlines()
    with open(val_sideview_path, encoding='utf-8') as f:
        val_sv_lines = f.readlines()
    num_train = len(train_tv_lines)
    num_val = len(val_tv_lines)
    ...
    if True:
        ...
        #   构建数据集加载器。
        # ---------------------------------------#
        train_dataset = YoloDataset(train_tv_lines, train_sv_lines, input_shape, num_classes, anchors, anchors_mask,
        ...
        val_dataset = YoloDataset(val_tv_lines, val_sv_lines, input_shape, num_classes, anchors, anchors_mask,
                                  epoch_length=UnFreeze_Epoch, \
        ...
        if local_rank == 0:
            eval_callback = EvalCallback(model, input_shape, anchors, anchors_mask, class_names, num_classes,
                                         val_tv_lines,val_sv_lines, log_dir, Cuda, \
