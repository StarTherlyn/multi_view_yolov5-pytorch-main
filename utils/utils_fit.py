...
def fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step,
    ...
    for iteration, batch in enumerate(gen):
        ...
        images, images_sv, targets, y_trues = batch[0], batch[1], batch[2],batch[3]
        with torch.no_grad():
            if cuda:
                images = images.cuda(local_rank)
                images_sv = images_sv.cuda(local_rank)
        ...
        if not fp16:
            ...
            #   前向传播
            # ----------------------#
            outputs = model_train(images, images_sv)
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        with torch.no_grad():
            ...
            #   前向传播
            # ----------------------#
            outputs = model_train_eval(images, images_sv)
