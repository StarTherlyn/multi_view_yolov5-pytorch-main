...
class YoloDataset(Dataset):
    def __init__(self, annotation_lines, sv_lines,input_shape, num_classes, anchors, anchors_mask, epoch_length, \
                 mosaic, mixup, mosaic_prob, mixup_prob, train, special_aug_ratio=0.7):
        super(YoloDataset, self).__init__()
        self.annotation_lines = annotation_lines
        self.sv_lines = sv_lines
    ...
    def __getitem__(self, index):
        ...
        # else:
        flip = self.rand() < .5
        image, box = self.get_random_data(self.annotation_lines[index], self.input_shape, flip=flip,random=self.train)
        image_add, _ = self.get_random_data(self.sv_lines[index], self.input_shape, flip=flip,random=self.train)

        image = np.transpose(preprocess_input(np.array(image, dtype=np.float32)), (2, 0, 1))
        image_add = np.transpose(preprocess_input(np.array(image_add, dtype=np.float32)), (2, 0, 1))
        ...
        return image, image_add, box, y_true

    def rand(self, a=0, b=1):
        return np.random.rand() * (b - a) + a

    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, flip=False,random=True):
